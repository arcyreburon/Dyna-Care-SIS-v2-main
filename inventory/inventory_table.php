...
<style>
    ...
    button.form-select.dropdown-toggle::after {
        margin-left: 0.5em;
    }
...
<main id="main" class="main">
                                            ...
                                            <div class="modal-body">
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-6 mb-3">
                                                        ...
                                                        <div class="dropdown">
                                                            <button class="form-select text-start dropdown-toggle" type="button" id="productDropdownBtn" data-bs-toggle="dropdown" aria-expanded="false">
                                                                <span id="selectedProductText">Select Product</span>
                                                            </button>
                                                            <input type="hidden" id="add_product_id" name="product_id" required>
                                                            
                                                            <div class="dropdown-menu dropdown-menu-lg p-2 w-100" aria-labelledby="productDropdownBtn" style="max-height: 300px; overflow-y: auto;">
                                                                <div class="mb-2 px-2">
                                                                    <input type="text" class="form-control" id="productSearchInput" placeholder="Search products..." autocomplete="off">
                                                                </div>
                                                                <div id="productDropdownItems">
                                                                    <div class="dropdown-item text-muted">Select a category first</div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div id="productError" class="invalid-feedback" style="display: none;"></div>
                                                    </div>
                                                    <div class="col-md-6 mb-3">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('addInventoryForm').addEventListener('submit', function(e) {
            ...
            if (!document.getElementById('add_product_id').value) {
                e.preventDefault();
                document.getElementById('productError').textContent = 'Please select a product';
                document.getElementById('productError').style.display = 'block';
                document.getElementById('productDropdownBtn').classList.add('is-invalid');
            }
        });

        // Initialize dropdown search functionality
        const productDropdown = new bootstrap.Dropdown(document.getElementById('productDropdownBtn'));
        
        // Function to load products with error handling
        async function loadProductsIntoDropdown(categoryId, searchTerm = '') {
            const dropdownItems = document.getElementById('productDropdownItems');
            ...
            if (!categoryId) {
                dropdownItems.innerHTML = '<div class="dropdown-item text-muted">Select a category first</div>';
                return;
            }

            // Show loading state
            dropdownItems.innerHTML = '<div class="dropdown-item text-muted">Loading products...</div>';

            try {
                const response = await fetch(`get_products.php?category_id=${categoryId}&search=${encodeURIComponent(searchTerm)}`);
                ...
                if (data.length === 0) {
                    dropdownItems.innerHTML = '<div class="dropdown-item text-muted">No products found matching your search</div>';
                    return;
                }
                ...
                data.forEach(product => {
                    ...
                    item.onclick = function() {
                        ...
                        document.getElementById('selectedProductText').textContent = product.product_name;
                        document.getElementById('productDropdownBtn').classList.remove('is-invalid');
                    };
                    dropdownItems.appendChild(item);
                });
            } catch (error) {
                console.error('Error loading products:', error);
                dropdownItems.innerHTML = '<div class="dropdown-item text-danger">Error loading products</div>';
        ...
        document.getElementById('add_category_id').addEventListener('change', function() {
            ...
            loadProductsIntoDropdown(categoryId);
        });

        document.getElementById('productSearchInput').addEventListener('input', function() {
            const categoryId = document.getElementById('add_category_id').value;
            loadProductsIntoDropdown(categoryId, this.value);
        });

        document.getElementById('productDropdownBtn').addEventListener('shown.bs.dropdown', function() {
